...
문서 요청에서 세션과 전역 모드를 병렬로 읽고 `SessionProvider`와 `PlatformModeProvider`의 초기 상태로 전달합니다. 각 조회는 최대 1초만 기다리며 실패한 항목은 브라우저가 기존 API로 다시 확인합니다. 사용자 상태가 포함된 문서는 동적 응답이며 공유 캐시에 저장하지 않습니다. 로그인 후에는 반드시 새 세션 요청이 끝나야 이동하고, 중첩 요청에서는 가장 최근 응답만 반영합니다.

홈은 `/api/home`에서 게시판·공지·랭킹·계정 요약을 한 번에 받습니다. 인증한 세션을 공용 조회 함수에 전달하므로 다른 route handler를 다시 호출하거나 알림 전체 목록·IGK 원장을 읽지 않습니다. 게시판 요약은 SQL에서 게시판당 글 ID 5개를 먼저 고른 뒤 필요한 관계를 조회합니다. 대화방 요약도 `LATERAL ... LIMIT 1`으로 방마다 최신 메시지 한 건만 읽습니다. Prisma 5의 중첩 `take`가 전체 글·메시지를 읽던 비용을 줄이며 기존 권한과 응답 형식을 유지합니다.

홈과 최근 대화는 `intact_cache_scope`로 분리된 `sessionStorage`에만 보관합니다. 같은 문서 안에서 이동할 때 최근 5개 방의 메시지 각 100개를 복원하고 실제 API·socket 상태로 갱신합니다. 새 문서의 서버 초기 상태를 적용할 때는 HTML 전송 중 모드가 바뀌었을 가능성에 대비해 현재 탭의 데이터 캐시를 먼저 비웁니다. 로그아웃·세션 만료·모드 변경 때도 삭제합니다. 서비스 워커와 공유 HTTP cache에는 사용자 데이터를 저장하지 않습니다. `/offline`은 세션 공급자를 사용하지 않는 별도 문서이며, 서비스 워커는 인증 쿠키 없이 이 문서만 저장합니다.

공통·게시판·검색 링크는 화면에 들어왔다는 이유로 미리 요청하지 않습니다. hover·focus·touch 의도가 감지될 때 route를 준비하고 대화 링크에서는 Socket.IO bundle도 준비합니다. 응답을 사용하지 않던 대화방 목록 사전 요청은 제거했습니다. 전역 모드 소켓과 대화 소켓은 동적 import로 분리하고, 알림 환경설정·푸시 설정은 설정 창을 처음 열 때 조회합니다. socket ACK가 1.5초 이상 지연될 때는 동일 idempotency key의 HTTP 요청을 병행하므로 두 경로 모두 중복 생성 방지를 유지해야 합니다.

새로 처리하는 이미지에는 크기·blur placeholder와 320/640/1280px WebP 파생본을 생성합니다. 기존 첨부는 썸네일 요청에서도 원본을 사용하며 요청 시 파생본을 생성하지 않습니다. 전체 글의 첫 이미지는 우선 요청하고 목록 썸네일은 지연 로딩합니다. 사진 뷰어는 별도 chunk로 열며 로딩 실패 시 다시 시도하거나 기존 미리보기 페이지로 이동할 수 있습니다. Web Vitals는 10%의 탭만 익명 sampling하며 콘텐츠, 사용자 ID와 query string을 기록하지 않습니다.

측정 조건·결과와 재현 방법은 `PERFORMANCE_REPORT.md`에 기록합니다.
...
- 클라이언트는 서버 초기 상태 이후 소켓 연결·재연결·무효화 알림과 탭 복귀 때 `/api/platform`을 확인합니다. 소켓 연결 여부와 관계없이 보이는 탭에서 5초마다 재확인하여 Redis·outbox 알림 누락에도 대응합니다. 모드 version이 바뀌면 session/chat cache를 비우고 다시 로드하며, 모드 확인 실패 시 개인정보 화면을 열지 않습니다.
