...
export default function IgkShopPage() {
  ...
  return <div className="mx-auto w-full max-w-[1200px] px-4 py-4 sm:px-6 lg:px-8">
    <PageHeading title="IGK 상점" description="실제 효과를 미리 보고 필요한 꾸미기만 고르세요. 구매로 현재 IGK가 줄면 등급과 짱 순위도 내려갈 수 있습니다." actions={<Link href="/igk" className="inline-flex h-9 items-center gap-2 border border-slate-300 bg-white px-4 text-xs font-semibold text-slate-700 hover:bg-slate-50"><ArrowLeft className="h-4 w-4" /> IGK 지갑</Link>} />
    ...
    {data ? <>
      <section className="mt-4 grid gap-px bg-emerald-950 sm:grid-cols-[1fr_auto]"><div className="bg-emerald-800 p-4 text-white"><p className="text-xs font-bold text-emerald-100">보유 IGK · {data.standing.tierLabel}{data.standing.rankLabel ? ` · ${data.standing.rankLabel}` : ''}</p><p className="mt-1 flex items-center gap-2 text-2xl font-bold"><Coins className="h-5 w-5" />{data.currentIgk.toLocaleString()}</p></div><div className="flex min-w-64 items-center gap-3 bg-emerald-900 px-4 py-3 text-sm text-emerald-50"><FlaskConical className="h-5 w-5" /><div><p className="font-bold">{SEASON_LABELS[data.activeSeason]} 컬렉션 판매 중</p><p className="text-xs text-emerald-200">서울 시간 기준 새 계절에 자동 교체</p></div></div></section>
      <div className="mt-4 flex flex-wrap items-center gap-2" aria-label="상점 필터">{([['all', '전체'], ['core', '상시'], ['season', `${SEASON_LABELS[data.activeSeason]} 한정`]] as const).map(([value, label]) => <Button key={value} variant={collection === value ? 'primary' : 'secondary'} onClick={() => setCollection(value)}>{label}</Button>)}<Button variant={ownedOnly ? 'primary' : 'secondary'} onClick={() => setOwnedOnly((value) => !value)}><Check className="h-4 w-4" />보유/장착만</Button></div>
