...
이 보고서의 측정 대상은 PR #81입니다. 운영 호환 릴리스는 `799518f`에 같은 성능 변경을 적용하며 기존 인증·채팅 응답 필드를 유지합니다. 운영 릴리스는 별도로 클라이언트 테스트 130개, lint·typecheck·production build와 운영 스키마의 migration 17개만 적용한 DB 통합 테스트 2개를 통과했습니다.
