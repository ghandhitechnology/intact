...
export default function ModerationConsole() {
  ...
  return (
    <main className="min-h-screen bg-[var(--surface-muted)] text-slate-950">
      <header className="border-b border-slate-800 bg-slate-950 text-white">
        <div className="mx-auto flex max-w-[1800px] items-center justify-between px-4 py-3 sm:px-6">
          <div className="flex items-center gap-3">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-700 shadow-[var(--shadow-sm)]"><ShieldCheck className="h-5 w-5" /></span>
            <div><h1 className="text-[15px] font-bold tracking-[-0.015em]">이중망 Moderation</h1><p className="text-[11px] font-semibold tracking-[0.14em] text-slate-400">LOCAL FORENSICS + CODEX LUNA</p></div>
          </div>
      ...
      <div className="mx-auto grid max-w-[1800px] items-start gap-4 p-4 sm:p-6 xl:grid-cols-[360px_minmax(0,1fr)_360px]">
        <Card className="overflow-hidden">
          <CardHeader title="심사 큐" description="불일치와 수동 검토 건을 우선 확인합니다." action={<Select value={state} onChange={(event) => setState(event.target.value)} className="h-9 w-36"><option value="ACTIVE">진행 중</option><option value="NEEDS_REVIEW">검토 필요</option><option value="ALLOWED">허용</option><option value="BLOCKED">차단</option><option value="SUPERSEDED">교체됨</option></Select>} />
          <div className="max-h-[calc(100vh-180px)] overflow-y-auto">
            {loading ? (
              ...
              <div className="px-4 py-12 text-center">
                <span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-slate-100 text-slate-400"><ShieldCheck className="h-5 w-5" /></span>
                <p className="mt-3 text-sm text-slate-500">해당 심사 건이 없습니다.</p>
              </div>
            ) : submissions.map((item) => (
              <button key={item.id} onClick={() => setSelectedId(item.id)} className={`block w-full border-b border-slate-100 p-4 text-left transition-colors last:border-b-0 ${selectedId === item.id ? "bg-emerald-50/70 shadow-[inset_3px_0_0_var(--green)]" : "hover:bg-slate-50/80"}`}>
                ...
                <p className="mt-2 truncate text-sm font-bold text-slate-900">{item.candidateTitle}</p><p className="mt-1 text-xs text-slate-500">{item.author.nickname} · {item.post.board.name}</p>
              </button>
            ))}
          </div>
        </Card>
        ...
        <div className="min-w-0 space-y-4">
          {!selected ? (
            <Card className="grid place-items-center p-12 text-center">
              <div>
                <span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-slate-100 text-slate-400"><ShieldCheck className="h-5 w-5" /></span>
                <p className="mt-3 text-sm text-slate-500">심사 건을 선택하세요.</p>
              </div>
            </Card>
          ) : <>
            <Card className="overflow-hidden"><CardHeader title={selected.candidateTitle} description={`${selected.author.realName || selected.author.nickname} · ${selected.isNewPost ? "새 게시물" : "공개본 수정"}`} action={<div className="flex gap-2"><Badge>{selected.state}</Badge>{selected.evasionDetected ? <Badge tone="red">우회 탐지</Badge> : null}</div>} />
              <div className="grid gap-0 md:grid-cols-2"><section className="border-b border-slate-100 p-4 md:border-b-0 md:border-r"><h3 className="text-xs font-bold text-slate-500">현재 공개본</h3><p className="mt-2 text-sm font-bold text-slate-900">{selected.post.title}</p><pre className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-600">{selected.post.contentText}</pre></section><section className="p-4"><h3 className="text-xs font-bold text-emerald-700">심사 후보 원문</h3><p className="mt-2 text-sm font-bold text-slate-900">{selected.candidateTitle}</p><pre className="mt-2 whitespace-pre-wrap text-xs leading-5 text-slate-700">{selected.candidateContentText}</pre></section></div>
            </Card>
            {selected.candidateAttachmentIds.length ? <Card className="overflow-hidden"><CardHeader title="재인코딩 이미지 증거" description="관리자 화면에도 메타데이터를 제거한 사본만 표시합니다." /><div className="grid grid-cols-2 gap-2.5 p-4 md:grid-cols-4">{selected.candidateAttachmentIds.map((id) => <Image key={id} src={`/api/admin/moderation/images/${id}`} alt="심사 이미지" width={320} height={320} unoptimized className="aspect-square w-full rounded-xl border border-slate-200 object-cover" />)}</div></Card> : null}
            <div className="grid items-start gap-4 lg:grid-cols-2"><Card className="overflow-hidden"><CardHeader title="복원 텍스트 / OCR" /><div className="space-y-4 p-4"><div><h3 className="text-xs font-bold text-slate-500">복원</h3><pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-950 p-3 text-xs leading-5 text-slate-100">{selected.normalizedText}</pre></div><div><h3 className="text-xs font-bold text-slate-500">OCR</h3><pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap rounded-xl bg-slate-50 p-3 text-xs leading-5">{selected.ocrText || "OCR 텍스트 없음"}</pre></div></div></Card><Card className="overflow-hidden"><CardHeader title="계층 판정 비교" description="불일치는 자동 게시되지 않습니다." /><div className="grid gap-3 p-4"><details open><summary className="cursor-pointer text-xs font-bold text-slate-700">LOCAL</summary><pre className="mt-2 max-h-64 overflow-auto rounded-xl bg-slate-950 p-3 text-xs leading-5 text-slate-100">{pretty(selected.localSignals)}</pre></details><details open><summary className="cursor-pointer text-xs font-bold text-slate-700">LUNA</summary><pre className="mt-2 max-h-64 overflow-auto rounded-xl bg-slate-950 p-3 text-xs leading-5 text-slate-100">{pretty(selected.lunaResult)}</pre></details></div></Card></div>
            <Card className="overflow-hidden"><CardHeader title="운영자 결정" description="모든 작업은 사유와 함께 감사 로그에 남습니다." /><div className="space-y-3 p-4"><Textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="판단 근거와 적용 규정을 기록하세요." rows={3} /><div className="flex flex-wrap gap-2"><Button onClick={() => void act("APPROVE")}>승인</Button><Button variant="danger" onClick={() => void act("REJECT")}>수정안 거절</Button><Button variant="danger" onClick={() => void act("HIDE")}>게시물 숨김</Button><Button variant="secondary" onClick={() => void act("RETRY")}>재검사</Button><Button variant="secondary" onClick={() => void act("SANCTION")}>경고 제재</Button></div></div></Card>
          </>}
        </div>
