...
export function LevelBadge({ level, jojinRank }: { level: number; jojinRank?: number | null }) {
  return (
  ...
  return (
    <span className="inline-flex min-w-0 items-center gap-1.5 text-xs text-slate-500">
      ...
      {member.studentId === "ADMIN" ? (
        ...
        <LevelBadge level={member.level} jojinRank={member.jojinRank} />
      )}
    </span>
  );
}
