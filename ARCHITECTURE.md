...
등급은 내부 level 1–10을 사용자 화면에서 `9등급`, `8등급` … `1등급`, `조졸`로 표시합니다. 조졸 중 보유 IGK 상위 8명은 `조진 · N짱`으로 표시합니다. 기준값은 `client/src/lib/igk-levels.ts`와 `LevelRule`에 맞춰 관리하고 `/igk/roadmap`에서 전체 기준을 공개합니다.
